...
`.gitignore` is defense-in-depth, and every engagement folder `mkengagement`
...
helpers only (`mkengagement`, `eng`, `logshell`, `nmapsweep`, `bhce`) —
...
```sh
mkengagement acme-external      # scaffold ~/engagements/YYYYMMDD-acme-external, open scope.txt
eng                             # fzf-jump between existing engagements; sets $ENGAGEMENT
cd "$ENGAGEMENT/recon"          # navigate into a subdir of the active engagement
logshell                        # record shell session (script + timing) into notes/ for audit
